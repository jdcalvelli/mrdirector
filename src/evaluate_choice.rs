...
pub fn evaluate_choice(state:&mut DirectorState, text_overlay: TextOverlay) {
    ...
    // render choice textbox
    text_overlay.render_choice_textbox(&choices);
    ...
    if gamepad(0).left.just_pressed() && choices.len() >= 1 {
        if diverts[0] == "NULL" {
            // pass entirely
            return;
        }
    ...
    else if gamepad(0).right.just_pressed() && choices.len() >= 2 {
        if diverts[1] == "NULL" {
            // pass entirely
            return;
        }
    ...
    else if gamepad(0).up.just_pressed() && choices.len() >= 3 {
        if diverts[2] == "NULL" {
            // pass entirely
            return;
        }
    ...
    else if gamepad(0).down.just_pressed() && choices.len() >= 4 {
        if diverts[3] == "NULL" {
            // pass entirely
            return;
        }
